...
/// Decrease liquidity directly through PoolManager using the callback pattern
///
...
async fn decrease_liquidity_pool_manager<P: Provider>(provider: P) -> eyre::Result<()> {
    // Define the pool to remove liquidity from
    ...
    // Amount of liquidity to remove (negative value for removing liquidity)
    // The negative value indicates a decrease in liquidity
    let liquidity_delta = I256::unchecked_from(-100000);

    // Salt for position identification (must match the salt used when adding
    // liquidity)
