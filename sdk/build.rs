...
const BINDINGS_PATH: &str = "sdk/src/types/contract_bindings/mod.rs";

const WANTED_CONTRACTS: [&str; 1] = ["UserPositionFetcher.sol"];
...
fn main() {
    ...
    let sol_macro_invocation = std::fs::read_dir(out_dir)
        ...
        .map(|(name, path_of_contracts)| {
            let path_of_contracts = path_of_contracts.replace(&format!("{this_dir}/"), "../");
...
"#
    ...
    let f_p = format!("{this_dir}/{BINDINGS_PATH}");
    let mut f = std::fs::File::options()
        .write(true)
        .truncate(true)
        .open(&f_p)
        .map_err(|e| eyre::eyre!("{f_p}\n{e:?}"))
        .unwrap();
