...
pub trait AngstromL2DataApi<N: Network>: PoolTickDataLoader<N> + Send + Sized {
    ...
    //     self.pool_key_by_pool_id_with_config_store(pool_id, config_store,
    // block_number)         .await
    // }
    ...
    //     let pools =
    // futures::future::try_join_all(token_pairs.into_iter().map(|pair| {
    //         self.pool_data_by_tokens(pair.token0, pair.token1, load_ticks,
    // block_number)     }))
    //     .await?;
